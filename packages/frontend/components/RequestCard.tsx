...
import { useRequestInfo, useHasSubmitted, useEscrow, useClaimDeposit } from "../hooks/useRequest";
import dynamic from "next/dynamic";
import clsx from "clsx";

const ProposalModal = dynamic(() => import("./BidModal").then(mod => mod.BidModal), { ssr: false });
...
export function RequestCard({ requestId, index }: Props) {
  const { address } = useAccount();
  const { data: info, refetch } = useRequestInfo(requestId);
  const { data: hasSubmitted } = useHasSubmitted(address, requestId);
  const { data: escrow } = useEscrow(address, requestId);
  ...
  if (!info) return <div className="glass-card h-64 shimmer" />;
      ...
      >
          ...
          >
            ...
            <span className="text-slate-300">
              ...
              {isWinner
                ...
                : <span className="font-mono text-slate-400">{bestVendor.slice(0, 8)}…{bestVendor.slice(-4)}</span>
              }
            </span>
          </div>
        )}
        ...
        <div className="flex gap-2 mt-auto">
          {isLive && !hasSubmitted && address && (
            <button onClick={() => setShowProposal(true)} className="btn-primary flex-1 flex items-center justify-center gap-2">
              <Lock size={14} /> Submit Proposal
            </button>
          )}
          {canClaim && (
            <button
              onClick={() => claimDeposit(requestId).then(() => refetch())}
              disabled={isClaiming}
          ...
          {!address && isLive && (
            <p className="flex-1 text-center text-sm text-slate-500 py-2">Connect wallet to submit</p>
          )}
          {hasSubmitted && isLive && (
            <p className="flex-1 text-center text-sm text-slate-500 py-2">Proposal submitted ✓</p>
          )}
        </div>
      </motion.div>
