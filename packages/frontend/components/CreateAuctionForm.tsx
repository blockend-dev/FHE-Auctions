    ...
    <form onSubmit={handleSubmit} className="card space-y-4">
      ...
      <div>
        ...
        <input
          ...
          placeholder="e.g. Cloud Migration, Marketing Agency…"
          value={title}
      ...
      <div>
        <label className="label">Weights (must sum to 100): Price {wPrice} · Quality {wQuality} · Delivery {wDelivery}</label>
        <div className="grid grid-cols-3 gap-2">
          {[["Price", wPrice, setWPrice], ["Quality", wQuality, setWQuality], ["Delivery", wDelivery, setWDelivery]].map(([lbl, val, set]) => (
            <input key={lbl as string} className="input w-full" type="number" min={0} max={100}
              value={val as number} onChange={(e) => (set as (v: number) => void)(Number(e.target.value))}
